...
export class FraudService {
  ...
  private transactionsCache$?: Observable<FraudTransaction[]>;

  constructor(private http: HttpClient) {}

  checkFraud(transaction: TransactionPayload): Observable<FraudTransaction> {
    return this.http
      .post<TransactionPayload>(`${this.FRAUD_API}/check`, this.prepareTransactionRequest(transaction))
      .pipe(
        timeout(this.TIMEOUT),
        map((response) => this.normalizeTransaction(response)),
        catchError(this.handleError.bind(this))
      );
  }

  getFraudTransactions(refresh = false): Observable<FraudTransaction[]> {
    return this.loadTransactions(refresh);
  }

  getDashboardStats(): Observable<DashboardStats> {
    return this.loadTransactions().pipe(
      map((transactions) => this.buildDashboardStats(transactions))
    );
  }

  getMerchantsList(): Observable<MerchantFraudSummary[]> {
    return this.loadTransactions().pipe(
      map((transactions) => this.buildMerchantSummaries(transactions))
    );
  }

  getCategoryFraudData(): Observable<CategoryFraud[]> {
    return this.loadTransactions().pipe(
      map((transactions) => this.buildCategoryFraud(transactions))
    );
  }

  getDailyFraudData(): Observable<DailyFraud[]> {
    return this.loadTransactions().pipe(
      map((transactions) => this.buildDailyFraud(transactions))
    );
  }

  getMockCategoryData(): CategoryFraud[] {
    return [
  ...
  private loadTransactions(refresh = false): Observable<FraudTransaction[]> {
    if (!this.transactionsCache$ || refresh) {
      this.transactionsCache$ = this.http.get<TransactionPayload[]>(`${this.FRAUD_API}/transactions`)
        .pipe(
          timeout(this.TIMEOUT),
          map((transactions) => Array.isArray(transactions) ? transactions.map((transaction) => this.normalizeTransaction(transaction)) : []),
          catchError((error) => {
  ...
  private prepareTransactionRequest(transaction: TransactionPayload): TransactionPayload {
    return {
  ...
  private normalizeTransaction(transaction: Partial<TransactionPayload> | null | undefined): FraudTransaction {
    ...
    return {
      ...
      transactionId: this.toText(
        safeTransaction.transactionId,
        `TXN${String(Math.abs(Math.round(amount)) + new Date(createdAt).getTime()).slice(-6).padStart(6, '0')}`
      ),
      senderName,
  ...
  private buildDashboardStats(transactions: FraudTransaction[]): DashboardStats {
  ...
  private buildMerchantSummaries(transactions: FraudTransaction[]): MerchantFraudSummary[] {
    ...
    const summaryMap = new Map<string, MerchantFraudSummary & { riskAccumulator: number }>();
  ...
  private buildCategoryFraud(transactions: FraudTransaction[]): CategoryFraud[] {
    const relevantTransactions = transactions.filter((transaction) => transaction.status !== 'NORMAL');
    const source = relevantTransactions.length > 0 ? relevantTransactions : transactions;
  ...
  private buildDailyFraud(transactions: FraudTransaction[]): DailyFraud[] {
    const groupedData = new Map<string, { sortValue: number; label: string; total: number; flagged: number; highRisk: number; mediumRisk: number }>();
  ...
  private getMockTransactions(): FraudTransaction[] {
    const channels: TransactionChannel[] = ['ONLINE', 'ATM', 'UPI', 'POS'];
  ...
  private normalizeTransactionType(value: unknown): TransactionType {
    return this.toText(value, 'DEBIT').toUpperCase() === 'CREDIT' ? 'CREDIT' : 'DEBIT';
  }

  private normalizeChannel(value: unknown): TransactionChannel {
  ...
  private normalizeStatus(value: unknown, riskScore: number): FraudStatus {
  ...
  private toRiskLevel(riskScore: number, status: FraudStatus): RiskLevel {
  ...
  private normalizeResponseDate(value: TransactionPayload['createdAt']): string {
  ...
  private normalizeRequestDate(value: TransactionPayload['createdAt']): string | undefined {
    if (typeof value === 'string' && value.trim()) {
      return value.trim();
    }
  ...
  private handleError(error: unknown): Observable<never> {
    ...
    if (typeof error === 'object' && error !== null) {
      const candidate = error as { error?: { message?: string } | ErrorEvent; status?: number; message?: string; name?: string };
